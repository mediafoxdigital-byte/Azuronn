...
function admin_build_product_attribute_overrides_from_post(array $existing): array
{
    ...
    return clean_product_library_item(array_merge($existing, [
        ...
        'option_band_claw_metal_options' => is_array($data['option_band_claw_metal_options'] ?? null) ? array_values($data['option_band_claw_metal_options']) : [],
...
<?php else: ?>
              ...
              <div class="admin-product-main">
                <section class="admin-product-card">
                  ...
                  <div class="admin-grid two-up">
                    ...
                    <?php admin_select('product[category_taxonomy]', 'Category', $categoryTaxonomySelected, $categoryTaxonomyOptions, 'required', 'Where this product appears on the storefront: Engagement Rings, Wedding Rings (Men\'s / Women\'s), Earrings, and so on.'); ?>
                    ...
                    <details class="admin-field admin-field-full" style="border:1px dashed #d8d2c4; border-radius:8px; padding:10px 14px;">
                      <summary style="cursor:pointer; font-size:0.8rem; letter-spacing:0.08em; text-transform:uppercase; color:#8a7a58;">Advanced: internal product type</summary>
                      <div class="admin-grid two-up" style="margin-top:12px;">
                        <?php admin_select('product[product_type]', 'Product Type', $editingProduct['product_type'] ?? $catalogEditorType, admin_options_from_list($productTypes), '', 'Only change this for custom categories. The Category field above normally sets this automatically.'); ?>
                      </div>
                    </details>
                  </div>
                </section>
                          ...
                          <?php foreach ($profileMetalOptions as $optionIndex => $option): ?>
                            <div class="admin-repeater-item compact-item">
                              ...
                              <div class="admin-grid two-up">
                                ...
                                <?php admin_input('product[option_metal_options][' . $optionIndex . '][price]', 'Price (£)', $option['price'] ?? '', 'number', 'step="0.01" min="0"', 'Per-metal base price shown on the product page.'); ?>
                              </div>
                              <div class="admin-grid two-up">
                          ...
                          <?php foreach ($profileBandOptions as $optionIndex => $option): ?>
                            <div class="admin-repeater-item compact-item">
                              ...
                              <div class="admin-grid two-up">
                                ...
                                <?php admin_input('product[option_band_claw_metal_options][' . $optionIndex . '][surcharge]', 'Surcharge (£)', $option['surcharge'] ?? '', 'number', 'step="0.01" min="0"', 'Extra cost added to the base price for this band option.'); ?>
                              </div>
                              <?php admin_input('product[option_band_claw_metal_options][' . $optionIndex . '][image]', 'Swatch Image URL', $option['image'] ?? '', 'url', '', 'Upload a swatch image to media library first, then paste the URL here. Shown on the product page when customer selects this band option.'); ?>
                          ...
                          <div class="admin-repeater-list">
                            <?php foreach ($productBandOptions as $optionIndex => $option): ?>
                              <div class="admin-repeater-item compact-item">
                                ...
                                <?php admin_input('product[option_band_claw_metal_options][' . $optionIndex . '][image]', 'Image URL', $option['image'] ?? '', 'url', '', 'Upload image to media library first, then paste the URL here.'); ?>
                                <?php admin_textarea('product[option_band_claw_metal_options][' . $optionIndex . '][description]', 'Description', $option['description'] ?? '', 3); ?>
                              </div>
                            <?php endforeach; ?>
                          </div>
  ...
  <template id="tpl-product-band-option">
    <div class="admin-repeater-item compact-item">
      ...
      <?php admin_input('product[option_band_claw_metal_options][__PRODUCT_BAND_INDEX__][image]', 'Image URL', '', 'url', '', 'Upload image to media library first, then paste the URL here.'); ?>
