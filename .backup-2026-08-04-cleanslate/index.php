...
function admin_build_attribute_profile_from_post(string $type, array $existing = []): array
{
    ...
    if (admin_product_type_is_ring($type)) {
        // Per-section showcases (engagement / wedding). Engagement is the primary
        // "Ring Style Showcase" in the admin UI and is mirrored to the legacy flat
        // style_cards list so existing fallback reads keep working.
        ...
        // Mirror the engagement list to the legacy flat style_cards field for
        // backward compatibility with any fallback reads in the storefront.
        $styleCards = $styleCardsSections['engagement'];
    }
          ...
          <section class="admin-panel admin-editor-panel" id="catalog-editor">
            ...
            <div class="admin-product-editor" data-catalog-profile-editor>
              <div class="admin-product-main">
                ...
                <?php
                  // Compute data-product-scope for "simple" (non-metal-matrix) cards
                  // so a merchant-created category always sees price/media/tags even
                  // though the same form is reused for matrix types. The matrix card
                  // still tags itself with the matrix scope below.
                  ...
                  $matrixScopeValues = [];
                  foreach (product_category_taxonomy_options() as $taxOptionForMatrix) {
                      $ptCanonM = admin_canonical_attribute_type((string) ($taxOptionForMatrix['product_type'] ?? ''));
                      if ($ptCanonM !== '' && admin_product_type_is_matrix($ptCanonM)) {
                          $matrixScopeValues[] = strtolower($ptCanonM);
                      }
                  }
                  $matrixScope = implode(', ', array_unique(array_filter($matrixScopeValues)));
                ?>
                ...
                <section class="admin-product-card" data-metal-matrix-card data-product-scope="<?= admin_html($matrixScope != '' ? $matrixScope : 'ring, rings') ?>">
                  <div class="admin-product-card-head">
                    <div>
              ...
              <section class="admin-product-card">
                ...
                <div class="admin-grid two-up">
                  ...
                  <?php if ($attributeTypeIsRing): ?>
                    ...
                    <?php
                      // Ring Style Showcase is the single primary style list for rings.
                      // It seeds the engagement section and is mirrored to the legacy
                      // flat style_cards field so existing fallback reads keep working.
                      $engagementSectionCards = array_values($attributeProfile['style_cards_sections']['engagement'] ?? []);
                      if ($engagementSectionCards === []) {
                          $engagementSectionCards = array_values($attributeProfile['style_cards'] ?? []);
                      }
                      if ($engagementSectionCards === []) {
                          $engagementSectionCards = array_values($defaultStyleCards);
                      }
                    ...
                    <section class="admin-field admin-field-full admin-attr-group" id="attribute-ring-styles">
                      <span><span class="admin-attr-group-num"><?= str_pad((string) $adminAttrGroupNum++, 2, '0', STR_PAD_LEFT) ?></span>Ring Style Showcase</span>
                      <p class="admin-table-note">The primary ring design bubbles shown across the live ring pages (engagement rings use this list). Rename the display text, change the image, or add new styles.</p>
                      <div class="admin-repeater" data-repeater data-index-token="__ENG_STYLE_INDEX__" data-next-index="<?= count($engagementSectionCards) ?>">
                        ...
                        <button class="admin-add" type="button" data-add-item data-template="tpl-product-style-card-engagement">Add Style</button>
                      </div>
                    </section>
                    <section class="admin-field admin-field-full admin-attr-group" id="attribute-wedding-styles">
                      <span><span class="admin-attr-group-num"><?= str_pad((string) $adminAttrGroupNum++, 2, '0', STR_PAD_LEFT) ?></span>Wedding Rings — Shop by Style</span>
                      <p class="admin-table-note">Styles shown for wedding rings (both men's and women's bands). This list is separate from the main Ring Style Showcase above.</p>
