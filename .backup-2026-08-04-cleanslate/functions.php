...
function available_ring_style_cards(string $ringCategory = ''): array
{
    ...
    // Only seed demo engagement styles when the merchant has never configured
    // anything. Once a real profile exists in site content, the profile is
    // authoritative and demo items must not reappear.
    $content = site_content();
    $profiles = is_array($content['catalog_meta']['attribute_profiles'] ?? null) ? $content['catalog_meta']['attribute_profiles'] : [];
    $hasRealRingProfile = isset($profiles['Ring']) || isset($profiles['Rings']);
    if ($cards === [] && $ringCategory !== 'wedding' && !$hasRealRingProfile) {
        foreach (default_ring_style_cards() as $styleKey => $defaultCard) {
            $cards[$styleKey] = [
                'value' => $styleKey,
 ...
 * real category cards created in the Categories admin so newly added
 * categories appear automatically. Ring-section cards resolve to the
 * structural ring taxonomy; every other real category becomes a usable entry
 * keyed by its canonical type. Well-known singular/plural aliases collapse to
 * one canonical product type so "Earrings" maps to "Earring" cleanly.
 */
function product_category_taxonomy_options(): array
{
    ...
    $typeAliases = [
        'rings' => 'Ring', 'engagement rings' => 'Ring', 'wedding rings' => 'Ring',
        'earrings' => 'Earring', 'bracelets' => 'Bracelet', 'bangles & bracelets' => 'Bracelet',
        'necklaces' => 'Necklace', 'pendants' => 'Pendant',
        'brooches' => 'Brooch', 'jewellery sets' => 'Jewellery Set', 'jewellery set' => 'Jewellery Set',
    ];
    $canonicalDisplayType = static function (string $title) use ($typeAliases): string {
        $lower = strtolower(trim($title));
        return $typeAliases[$lower] ?? trim($title);
    };

    $ringSectionTitles = ['engagement rings', 'wedding rings', "women's wedding rings", "men's wedding rings", 'ring', 'rings'];

    foreach ((array) (site_content()['category_cards'] ?? []) as $card) {
        if (!is_array($card)) {
            continue;
        }
        ...
        if (in_array($lower, $ringSectionTitles, true)) {
            $options['engagement']['label'] = $title;
            continue;
        }
        $typeKey = 'custom-' . strtolower((string) preg_replace('/[^a-z0-9]+/i', '-', $title));
        $options[$typeKey] = [
            'label' => $title,
            'product_type' => $canonicalDisplayType($title),
            'ring_category' => '',
            'ring_gender' => '',
        ];
    }

    // Include any canonical product type already in use by products or
    // attribute profiles so editing never loses its category.
    $content = site_content();
    $types = (array) ($content['catalog_meta']['product_types'] ?? []);
    foreach ((array) ($content['products']['items'] ?? []) as $prod) {
        $t = clean_string((string) ($prod['product_type'] ?? ''), 80);
        if ($t !== '') {
            $types[] = $t;
        }
    }
    foreach (array_unique(array_filter($types)) as $type) {
        ...
        $canonical = $typeAliases[$norm] ?? trim((string) $type);
        if ($canonical === '' || in_array(strtolower($canonical), ['ring', 'rings'], true)) {
            continue;
        }
        $alreadyPresent = false;
        foreach ($options as $opt) {
            if (strtolower(trim((string) ($opt['product_type'] ?? ''))) === strtolower($canonical)) {
                $alreadyPresent = true;
                break;
            }
        }
        if (!$alreadyPresent) {
            $options['custom-' . strtolower((string) preg_replace('/[^a-z0-9]+/i', '-', $canonical))] = [
                'label' => $canonical,
...
function available_collection_selector_cards(string $type): array
{
    ...
    // Only when the merchant has never configured this category do we offer
    // the demo seed cards — stripped of hardcoded demo product IDs so faceted
    // filtering doesn't silently resolve against non-existent products.
    if ($cards === []) {
        $content = site_content();
        $profiles = is_array($content['catalog_meta']['attribute_profiles'] ?? null) ? $content['catalog_meta']['attribute_profiles'] : [];
        $hasRealProfile = isset($profiles[$type]);
        if (!$hasRealProfile) {
            foreach (default_collection_selector_cards($type) as $value => $defaultCard) {
                $cards[$value] = [
                    'value' => $value,
                    'label' => $defaultCard['label'] ?? $value,
                    'image' => $defaultCard['image'] ?? '',
                    'product_ids' => [], // never carry demo IDs
                ];
            }
        }
    }
