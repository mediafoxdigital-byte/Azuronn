...
function admin_product_options(array $products): string
{
    ...
    foreach ($products as $product) {
        $label = trim(($product['name'] ?? 'Product') . ' - ' . ($product['category'] ?? '')); 
...
function admin_build_attribute_profile_from_post(string $type, array $existing = []): array
{
    ...
    if (admin_product_type_is_ring($type) && is_array($data['style_cards'] ?? null)) {
        $styleCards = [];
        $existingStyleCardsByValue = [];
        foreach ((array) ($existing['style_cards'] ?? []) as $existingCard) {
            if (!is_array($existingCard)) {
                continue;
            }
            $existingValue = clean_string((string) ($existingCard['value'] ?? ''), 80);
            if ($existingValue !== '') {
                $existingStyleCardsByValue[$existingValue] = $existingCard;
            }
        }
        foreach (array_values($data['style_cards']) as $index => $card) {
            if (!is_array($card)) {
                continue;
            }
            ...
            $existingCard = $existingStyleCardsByValue[$value] ?? ($existing['style_cards'][$index] ?? []);
            $styleCards[] = [
                'value' => $value,
                'label' => $card['label'] ?? '',
                'image' => admin_select_image_or_url('style_card_image_url_' . $index, 'style_card_image_file_' . $index, (string) ($card['current_image'] ?? ($existingCard['image'] ?? ''))),
            ];
        }
    }
...
function admin_apply_request_payload(array &$content, array $request): string
{
    ...
    switch ($action) {
        ...
        case 'save-catalog-assignments':
            ...
            $content['shop_by_style']['style_ids'] = clean_select_ids(is_array($assigned['shop_by_style'] ?? null) ? $assigned['shop_by_style'] : [], $styleOptionIds);
            save_site_content($content);
...
$productTypes = array_values(array_filter(array_map(static fn($c) => $c['title'] ?? '', $content['category_cards'] ?? [])));
...
$attributeTypes = $productTypes !== [] ? $productTypes : array_keys(default_attribute_profiles());
$attributeTypeFilter = clean_string($_GET['type'] ?? ($attributeTypes[0] ?? 'Ring'), 80);
if (!in_array($attributeTypeFilter, $attributeTypes, true)) {
    $attributeTypeFilter = $attributeTypes[0] ?? 'Ring';
}
...
$attributeProducts = array_values(array_filter($products, static function (array $product) use ($attributeTypeFilter): bool {
    return (string) ($product['product_type'] ?? '') === $attributeTypeFilter;
}));
...
if ($attributeEditingProduct !== null && (string) ($attributeEditingProduct['product_type'] ?? '') !== $attributeTypeFilter) {
    ...
    <main class="admin-main admin-main-v2">
      ...
      <?php elseif ($view === 'catalog'): ?>
        ...
        <?php if ($productCreate || $editingProduct !== null): ?>
          <?php
          $catalogEditorType = clean_string((string) ($editingProduct['product_type'] ?? ($productTypes[0] ?? 'Ring')), 80);
              ...
              <div class="admin-product-main">
                <section class="admin-product-card">
                  ...
                  <div class="admin-grid two-up">
                    ...
                    <?php admin_select('product[product_type]', 'Product Type', $editingProduct['product_type'] ?? $catalogEditorType, admin_options_from_list($productTypes), 'required', 'Changing type updates the available options below.'); ?>
                    <?php admin_input('product[category]', 'Category Label', $editingProduct['category'] ?? '', 'text', '', 'e.g. Women, Everyday, Statement, Bridal'); ?>
              ...
              <tbody>
                <?php foreach ($filteredProducts as $product): ?>
                  <tr>
                    ...
                    <td><?= admin_html($product['product_type'] ?? '') ?></td>
                    <?php
                      $activeMetals = [];
                      $minPrice = null;
                  ...
                  <?php foreach ($products as $product): ?>
                    <label class="admin-check-card">
                      ...
                      <span class="admin-check-media">
                        ...
                        <span class="admin-check-copy"><strong><?= admin_html($product['name']) ?></strong><small><?= admin_html(trim(($product['product_type'] ?? '') . ' • ' . ($product['color'] ?? '') . ' • ' . ($product['category'] ?? ''))) ?></small><em><?= admin_html($product['new_price']) ?></em></span>
                      </span>
                    </label>
                  <?php endforeach; ?>
                </div>
                  ...
                  <label class="admin-check-card">
                    ...
                    <span class="admin-check-media">
                      ...
                      <span class="admin-check-copy">
                        ...
                        <em><?= admin_html((string) ($styleOption['type'] ?? '')) ?></em>
                      </span>
                    </span>
                  </label>
                <?php endforeach; ?>
        ...
        <section class="admin-page-hero admin-page-hero-actions">
          ...
          <div class="admin-pill-row">
            <?php foreach ($attributeTypes as $attributeType): ?>
              <a class="admin-pill <?= $attributeType === $attributeTypeFilter ? 'is-active' : '' ?>" href="<?= admin_html(admin_url('attributes', ['type' => $attributeType])) ?>"><?= admin_html($attributeType) ?></a>
            <?php endforeach; ?>
          </div>
        </section>
              ...
              <article class="admin-list-card">
                <div class="admin-table-media">
                  ...
                  <div>
                    ...
                    <small><?= admin_html((string) ($product['category'] ?? '')) ?> • <?= admin_html((string) ($product['new_price'] ?? '')) ?></small>
                  </div>
                </div>
                <div class="admin-list-meta">
                    ...
                    <div class="admin-grid two-up">
                      <div class="admin-field admin-field-full">
                        ...
                        <div class="admin-choice-grid">
                          ...
                          <?php foreach (available_ring_styles() as $styleKey => $styleLabel): ?>
