...
if ($showRingJourney && $filters['gender'] !== '') {
    ...
    $filteredProducts = array_values(array_filter($filteredProducts, static function (array $product) use ($requestedGender): bool {
        $gender = product_ring_taxonomy($product)['gender'];
        return $gender === $requestedGender || $gender === 'unisex';
    }));
}
$isSearch = $filters['q'] !== '';
...
// have results. Built from the section-filtered products before style/color
// narrowing so every listed metal stays selectable.
...
$productColors = array_values(array_filter(array_unique(array_map(
    ...
    $sectionProductsForMetals
))));
sort($productColors);

// Apply the selected metal now that the option list is built.
if ($filters['color'] !== '') {
    $filteredProducts = array_values(array_filter($filteredProducts, static function (array $product) use ($filters): bool {
        return strcasecmp((string) ($product['color'] ?? ''), $filters['color']) === 0;
    }));
}
...
<?php if ($showRingJourney): ?>
  ...
  <?php if ($filters['ring_category'] !== 'wedding'): ?>
    ...
    <div class="style-selector-row ring-style-selector-row">
      ...
      ?>
        <form method="get" action="<?= h(resolve_link('/shop/')) ?>" class="style-selector-form">
          ...
          <?php if ($filters['ring_category'] !== ''): ?><input type="hidden" name="ring_category" value="<?= h($filters['ring_category']) ?>"><?php endif; ?>
          <?php if ($filters['gender'] !== ''): ?><input type="hidden" name="gender" value="<?= h($filters['gender']) ?>"><?php endif; ?>
          ...
          <button type="submit" class="style-selector-item <?= $isActive ? 'is-active' : '' ?>" title="<?= h($isActive ? 'Show all styles' : 'Show ' . $displayName . ' rings') ?>">
    ...
    <div class="shop-results">
      ...
      <?php if ($activeFilterPills !== []): ?>
        <div class="shop-active-filters" style="margin-bottom: 25px; display: flex; gap: 10px; flex-wrap: wrap;">
          ...
          <?php endforeach; ?>
          <a href="<?= h($buildShopUrl(['q' => '', 'type' => '', 'ring_category' => '', 'gender' => '', 'color' => '', 'category' => '', 'shape' => '', 'style' => '', 'facet' => '', 'sort' => 'featured'])) ?>" style="font-size: 0.8rem; color: #c9a96e; text-decoration: underline; margin-left: 10px; align-self: center;">Clear All</a>
        </div>
      <?php endif; ?>
      ...
      <?php if ($filteredProducts === []): ?>
        <div class="shop-empty-state" style="background: #fbfcfb; border: 1px solid #edf0ed; border-radius: 12px; padding: 60px 30px; text-align: center; margin: 40px 0;">
          ...
          <a class="btn-shop" href="<?= h($buildShopUrl(['q' => '', 'type' => '', 'ring_category' => '', 'gender' => '', 'color' => '', 'category' => '', 'shape' => '', 'style' => '', 'facet' => '', 'sort' => 'featured'])) ?>" style="background: #b18861; color: #fff; border: none; font-size: 0.8rem; letter-spacing: 0.15em; padding: 12px 30px; border-radius: 4px; text-transform: uppercase; font-weight: 600;">View All Products</a>
        </div>
      <?php else: ?>
