...
{
    $items = array_values(array_filter($content['newsletter']['subscribers'] ?? [], 'is_array'));
...
{
    $normalized = strtolower(clean_string($type, 80));
    $map = [
        'ring' => 'Ring', 'rings' => 'Ring',
        'earring' => 'Earring', 'earrings' => 'Earring',
        'pendant' => 'Pendant', 'pendants' => 'Pendant',
        'bracelet' => 'Bracelet', 'bracelets' => 'Bracelet', 'bangles & bracelets' => 'Bracelet',
        'necklace' => 'Necklace', 'necklaces' => 'Necklace',
        'brooch' => 'Brooch', 'brooches' => 'Brooch',
        'jewellery set' => 'Jewellery Set', 'jewellery sets' => 'Jewellery Set',
        'mangalsutra' => 'Mangalsutra',
    ];

    return $map[$normalized] ?? clean_string($type, 80);
}
...
function admin_handle_image_upload(string $fieldName, string $current = ''): string
{
    ...
    if (($file['size'] ?? 0) > 10 * 1024 * 1024) {
        admin_set_flash('error', 'File size exceeds 10MB limit.');
        return $current;
    }
...
function admin_select_image_or_url(string $urlField, string $fileField, string $current = ''): string
{
    ...
    // clean_image() already normalises a bare '#' to '', so the old check
    // ($url !== '#') was dead: an EMPTY url field fell through to returning ''
    // and WIPED the stored image on every save that didn't re-attach a file.
    // The live Site-Settings hero form always submits an empty url field, so the
    // image never persisted ("upload shows success but the image won't stick").
    // Preserve the existing image unless a new url/file was actually supplied.
...
function admin_diamond_profile_keys(array $content): array
{
    ...
    if (is_array($profiles['Rings'] ?? null)) {
        $keys[] = 'Rings';
    }
    if (is_array($profiles['Ring'] ?? null)) {
        $keys[] = 'Ring';
    }
    if ($keys === []) {
        $keys[] = 'Ring';
...
function admin_build_product_from_post(array $existing = [], int $index = 0): array
{
    ...
    return clean_product_library_item([
        ...
        'styles' => $isMatrixType
            ? (array_key_exists('styles', $data) && is_array($data['styles']) ? $data['styles'] : ($existing['styles'] ?? []))
            : [],
...
function admin_build_product_attribute_overrides_from_post(array $existing): array
{
    ...
    $isMatrixType = admin_product_type_is_matrix((string) infer_product_type($existing));
...
function admin_build_attribute_profile_from_post(string $type, array $existing = []): array
{
    ...
    if (admin_product_type_is_ring($type)) {
        $styleCards = $buildStyleCards(is_array($data['style_cards'] ?? null) ? $data['style_cards'] : null, (array) ($existing['style_cards'] ?? []), 'style_card_');
        // Per-section showcases (engagement / wedding) are optional; a section
        // keeps its existing cards unless its repeater was posted.
        foreach (['engagement', 'wedding'] as $ringSection) {
            $sectionExisting = is_array($styleCardsSections[$ringSection] ?? null) ? $styleCardsSections[$ringSection] : [];
            $posted = is_array($data['style_cards_sections'][$ringSection] ?? null) ? $data['style_cards_sections'][$ringSection] : null;
            $styleCardsSections[$ringSection] = $buildStyleCards($posted, $sectionExisting, 'style_card_' . $ringSection . '_');
        }
    }

    $selectorCards = $existing['selector_cards'] ?? [];
    if (!admin_product_type_is_ring($type) && is_array($data['selector_cards'] ?? null)) {
        $selectorCards = [];
        ...
        foreach (array_values($data['selector_cards']) as $index => $card) {
            if (!is_array($card)) {
                continue;
            }
...
function admin_prepare_request_payload(string $action, array $content): ?array
{
    return match ($action) {
        ...
        'ban-customer', 'unban-customer', 'delete-customer' => (function () use ($content, $action) {
            $customerId = clean_string($_POST['customer_id'] ?? '', 80);
            $index = admin_array_find_index($content['customers']['items'], $customerId);
            if ($index === null) {
                return null;
            }
            $before = $content['customers']['items'][$index];
            $after = $before;
        ...
        'resolve-order-request' => (function () use ($content) {
            $orderId = clean_string($_POST['order_id'] ?? '', 80);
...
function admin_apply_request_payload(array &$content, array $request): string
{
    ...
    switch ($action) {
        ...
        case 'save-attribute-profile':
            ...
            if ($renameMap !== []) {
                ...
                foreach ($content['products']['items'] as &$productItem) {
                    $productTypeLower = strtolower(trim((string) ($productItem['product_type'] ?? '')));
        ...
        case 'unban-customer':
            ...
            $content['customers']['items'][$index]['status'] = $action === 'ban-customer' ? 'banned' : 'active';
            save_site_content($content);
            return $action === 'ban-customer' ? 'User banned.' : 'User restored.';
        case 'delete-customer':
            $customerId = clean_string((string) ($payload['target_id'] ?? ''), 80);
            $content['customers']['items'] = array_values(array_filter($content['customers']['items'], static fn (array $item): bool => (string) ($item['id'] ?? '') !== $customerId));
            save_site_content($content);
        ...
        case 'resolve-order-request':
            ...
            $requestType = strtolower((string) ($content['orders']['items'][$index]['customer_request_type'] ?? ''));
            ...
            if ($resolution === 'reject' && in_array($requestStatus, ['pending', 'approved'], true)) {
                $content['orders']['items'][$index]['customer_request_status'] = 'rejected';
                $content['orders']['items'][$index]['customer_request_resolved_at'] = $now;
                save_site_content($content);
                ...
                if (in_array($requestType, ['cancel', 'return'], true)) {
                    ...
                    if ($resolution === 'approve' && $requestStatus === 'pending') {
                        ...
                        if ($requestType === 'cancel') {
                            ...
                            $content['orders']['items'][$index]['status'] = 'return-approved';
                        }
...
$ringSectionLabels = ['engagement rings', 'wedding rings'];
$productTypes = array_values(array_filter(array_map(static fn($c) => $c['title'] ?? '', $content['category_cards'] ?? []), static function ($title) use ($ringSectionLabels): bool {
    $title = strtolower(trim((string) $title));
    return $title !== '' && !in_array($title, $ringSectionLabels, true);
}));
foreach ($products as $catalogProduct) {
    $usedType = clean_string((string) ($catalogProduct['product_type'] ?? ''), 80);
    if ($usedType !== '' && !in_array($usedType, $productTypes, true)) {
        $productTypes[] = $usedType;
    }
}
$ringProfile = $content['catalog_meta']['attribute_profiles']['Ring'] ?? [];
$productMetals = array_values(array_filter(array_map(static fn($m) => $m['label'] ?? '', $ringProfile['metals'] ?? [])));
...
$rawAttributeTypes = $productTypes !== [] ? $productTypes : array_keys(default_attribute_profiles());
$attributeTypes = [];
$attributeTypeLabels = [];
...
$attributeProducts = array_values(array_filter($products, static function (array $product) use ($attributeTypeFilter): bool {
    return admin_canonical_attribute_type((string) ($product['product_type'] ?? '')) === $attributeTypeFilter;
}));
...
if ($attributeEditingProduct !== null && admin_canonical_attribute_type((string) ($attributeEditingProduct['product_type'] ?? '')) !== $attributeTypeFilter) {
...
$filteredCustomers = array_values(array_filter($content['customers']['items'] ?? [], static function (array $user) use ($customerStatusFilter, $customerQuery): bool {
...
$totalRevenue = array_reduce($content['orders']['items'] ?? [], static function (float $carry, array $order): float {
    ...
    <main class="admin-main admin-main-v2">
      ...
      <?php if ($view === 'dashboard'): ?>
        <section class="admin-page-hero admin-page-hero-dashboard">
          ...
          <div class="admin-mini-stats">
            ...
            <article><span>Orders</span><strong><?= count($content['orders']['items']) ?></strong></article>
            <article><span>Upcoming Appts</span><strong><?= count($apUpcoming ?? []) ?></strong></article>
            <article><span><?= admin_html(admin_is_employee_portal() ? 'My Pending Requests' : 'Open Requests') ?></span><strong><?= admin_html((string) (admin_is_employee_portal() ? count($pendingAdminRequests) : count(array_filter($content['orders']['items'], static fn (array $item): bool => in_array(strtolower((string) ($item['customer_request_status'] ?? '')), ['pending', 'approved'], true))))) ?></strong></article>
          </div>
        </section>
        ...
        <section class="admin-workflow-grid">
          ...
          <article class="admin-module-card">
            <h3>Orders</h3>
            <p><?= admin_metric_total($content['orders']['items'], 'status', 'pending') ?> pending orders and <?= count(array_filter($content['orders']['items'], static fn (array $item): bool => in_array(strtolower((string) ($item['customer_request_status'] ?? '')), ['pending', 'approved'], true))) ?> customer requests need attention.</p>
        ...
        <section class="admin-dashboard-grid">
          <section class="admin-panel">
            ...
            <div class="admin-list-stack">
              <?php foreach (array_slice($content['orders']['items'] ?? [], 0, 5) as $order): ?>
          ...
          <section class="admin-panel">
            ...
            <div class="admin-list-stack">
              <?php foreach (array_slice($content['customers']['items'] ?? [], 0, 5) as $customer): ?>
                <article class="admin-list-card">
                  <div>
      ...
      <?php elseif ($view === 'catalog'): ?>
        ...
        <?php if ($productCreate || $editingProduct !== null): ?>
          <?php
          $catalogEditorType = clean_string((string) ($editingProduct['product_type'] ?? ($defaultAttributeType ?? ($productTypes[0] ?? 'Ring'))), 80);
          if ($catalogEditorType === '') {
              $catalogEditorType = 'Ring';
          }
          // The Metal Matrix + attribute profile are keyed by the CANONICAL type
          // (Ring, not Rings). Normalise here so the editor context, the matrix
          // block that is shown, and the metal_variations_<type> POST key all
          // agree — and so the Category dropdown (which maps to a canonical type
          // below) can drive them without a Rings/Ring mismatch dropping metals.
          $catalogEditorType = admin_canonical_attribute_type($catalogEditorType);
          ...
          $catalogEditorColorChoices = admin_editor_list($catalogEditorSource, $catalogEditorProfile, 'option_color_choices');
          $catalogEditorSizeChoices = admin_editor_list($catalogEditorSource, $catalogEditorProfile, 'option_size_choices');
          $catalogEditorMetalOptions = admin_editor_list($catalogEditorSource, $catalogEditorProfile, 'option_metal_options');
          $catalogEditorBandOptions = admin_editor_list($catalogEditorSource, $catalogEditorProfile, 'option_band_claw_metal_options');
              ...
              <div class="admin-product-main">
                <section class="admin-product-card">
                  ...
                  <div class="admin-grid two-up">
                    ...
                    <?php if (!admin_product_type_is_matrix($catalogEditorType)): ?>
                      <?php admin_select('product[color]', 'Base Color', $editingProduct['color'] ?? '', admin_options_from_list($productColors ?? []), '', 'Default colour before customer selection.'); ?>
                    <?php endif; ?>
                    ...
                    <?php
                      ...
                      $initialProductType = admin_canonical_attribute_type((string) ($editingProduct['product_type'] ?? $catalogEditorType));
                ...
                <section class="admin-product-card">
                  ...
                  <div class="admin-grid three-up">
                    <?php admin_input('product[new_price]', 'Current Selling Price', $editingProduct['new_price'] ?? '', 'text', '', 'Example: £95.00'); ?>
                ...
                <section class="admin-product-card">
                  <div class="admin-product-card-head">
                    ...
                    <a href="<?= admin_html(admin_url('attributes', ['type' => $catalogEditorType])) ?>" class="admin-attr-edit-link"><i class="fas fa-sliders"></i> Edit <?= admin_html($catalogEditorType) ?> Attributes</a>
                  </div>
                  <p class="admin-table-note">Tick the options from the <strong><?= admin_html($catalogEditorType) ?></strong> attribute profile that apply to this product. Manage the master list in Attributes.</p>
                  <div class="admin-grid two-up" style="margin-bottom:18px">
                    <?php admin_input('product[option_color_label]', 'Color Section Label', $catalogEditorColorLabel, 'text', '', 'e.g. Metal, Color'); ?>
                    <?php admin_input('product[option_size_label]', 'Size Section Label', $catalogEditorSizeLabel, 'text', '', 'e.g. Size, Carat Weight'); ?>
                  </div>

                  <div class="admin-grid">
                    <div class="admin-profile-selector-col">
                      <div class="admin-profile-selector-head">
                        <span class="admin-profile-selector-label"><?= admin_html($catalogEditorSizeLabel ?: 'Size') ?> Options</span>
                        <small><?= count($catalogEditorProfile['option_size_choices'] ?? []) ?> available</small>
                      </div>
                      <?php if (!empty($catalogEditorProfile['option_size_choices'])): ?>
                      <div class="admin-profile-chips">
                        <?php foreach ($catalogEditorProfile['option_size_choices'] as $sIdx => $choice):
                          $isSel = $productCreate ? true : admin_choice_in_list($choice, $catalogEditorSizeChoices);
                        ?>
                      ...
                      <?php else: ?>
                      <p class="admin-empty-note">No size options in <strong><?= admin_html($catalogEditorType) ?></strong> profile yet. <a href="<?= admin_html(admin_url('attributes', ['type' => $catalogEditorType])) ?>">Add in Attributes →</a></p>
                      <?php endif; ?>
                    </div>
                  </div>
                </section>
                ...
                <section class="admin-product-card" data-product-scope="ring, rings, earring, earrings, bracelet, bangles & bracelets, necklace, necklaces, pendant, pendants">
                  <div class="admin-product-card-head">
                    <div>
                  ...
                  <?php
                    // ── Style picker (rings only) ──────────────────────────────
                    // Lets the merchant tag which ring styles (from this category's
                    // Attributes) this product belongs to. The list follows the
                    // chosen Category's ring section (engagement vs wedding) so a
                    // wedding band is offered wedding styles, not "Solitaire".
                    // Posts the SAME product[styles][] field the save handler + the
                    // storefront already read, so no flow change. The two section
                    // containers are toggled (and the hidden one's inputs disabled
                    // so it never submits) by admin.js syncProductStyleSection().
                    $initialStyleSection = in_array((string) ($editingProduct['ring_category'] ?? ''), ['engagement', 'wedding'], true)
                        ? (string) $editingProduct['ring_category']
                        : 'engagement';
                    $savedProductStyles = array_values((array) ($editingProduct['styles'] ?? []));
                    $styleSectionsForPicker = ['engagement', 'wedding'];
                  ?>
                  <div class="admin-field admin-field-full" data-product-scope="ring, rings" data-style-picker-wrap style="margin-bottom:18px;">
                    <span style="font-weight:600; font-size:0.95em; margin-bottom:4px; display:block;">Ring Style</span>
                    ...
                    <?php foreach ($styleSectionsForPicker as $pickerSection):
                        $pickerVisible = ($pickerSection === $initialStyleSection);
                    ?>
                      <div class="admin-choice-grid" data-style-section="<?= admin_html($pickerSection) ?>" style="<?= $pickerVisible ? '' : 'display:none;' ?>">
                        <?php // Empty sentinel so an unchecked group still submits
                              // product[styles][] (as ['']), letting the save store an
                              // empty list instead of falling back to the previous value.
                              // Disabled together with the grid when this section is hidden. ?>
                        <input type="hidden" name="product[styles][]" value="" <?= $pickerVisible ? '' : 'disabled' ?>>
                        <?php foreach (available_ring_styles($pickerSection) as $styleKey => $styleLabel): ?>
                  ...
                  <div class="admin-metal-matrix">
                    <?php 
                      $matrixProfiles = array_filter($catalogAttributeProfilesForJs, static fn ($pType) => admin_product_type_is_matrix((string) $pType), ARRAY_FILTER_USE_KEY);
                      foreach ($matrixProfiles as $pType => $pProfile):
                          $globalMetals = $pProfile['option_metal_options'] ?? [];
                          $globalBands = $pProfile['option_band_claw_metal_options'] ?? [];
                          $globalShapes = available_diamond_shapes();
                    ...
                    <div data-matrix-profile-type="<?= admin_html(strtolower((string) $pType)) ?>" style="display: <?= $isCurrentType ? 'block' : 'none' ?>;">
                      ...
                      <div class="admin-metal-matrix-block" style="padding:16px; border:1px solid #e0e6eb; border-radius:8px; margin-bottom:20px;">
                        ...
                        <div class="admin-grid two-up" style="margin-top:16px;">
                          <div class="admin-field" data-product-scope="ring, rings">
                        ...
                        <div class="admin-grid" style="margin-top:16px;" data-product-scope="ring, rings">
                          <div class="admin-field">
      ...
      <?php elseif ($view === 'attributes'): ?>
        <?php
          $defaultStyleCards = [];
          $savedStyleCards = [];
          if ($attributeTypeIsRing) {
              $defaultStyleCards = default_ring_style_cards();
              foreach (array_values($attributeProfile['style_cards'] ?? []) as $savedCard) {
                  if (!is_array($savedCard)) {
                      continue;
                  }
                  $savedValue = clean_string((string) ($savedCard['value'] ?? ''), 80);
                  if ($savedValue !== '') {
                      $savedStyleCards[$savedValue] = $savedCard;
                  }
              }
          }
        ?>
              ...
              <section class="admin-product-card">
                ...
                <div class="admin-grid two-up">
                  ...
                  <?php if ($attributeTypeIsRing): ?>
                    ...
                    <div class="admin-field admin-field-full" id="attribute-ring-styles">
                      <span>Ring Style Showcase</span>
                      <p class="admin-table-note">Control the ring design bubbles shown on the live ring page. You can rename the display text, change the image, or add completely new style cards.</p>
                      <?php
                        $profileStyleCards = array_values($attributeProfile['style_cards'] ?? []);
                        if ($profileStyleCards === []) {
                            $profileStyleCards = array_values($defaultStyleCards);
                        }
                      ?>
                      <div class="admin-repeater" data-repeater data-index-token="__PRODUCT_STYLE_INDEX__" data-next-index="<?= count($profileStyleCards) ?>">
                        <div class="admin-repeater-list">
                          <?php foreach ($profileStyleCards as $styleIndex => $styleCard): ?>
                            <?php
                              $styleValue = clean_string((string) ($styleCard['value'] ?? ''), 80);
                              $styleLabel = clean_string((string) ($styleCard['label'] ?? ''), 120);
                              $styleImage = clean_image((string) ($styleCard['image'] ?? ''));
                            ?>
                            <div class="admin-repeater-item compact-item">
                              <div class="admin-item-head"><h4><?= admin_html($styleLabel !== '' ? $styleLabel : 'Style ' . ($styleIndex + 1)) ?></h4><button class="admin-remove" type="button" data-remove-item>Delete</button></div>
                              <input type="hidden" name="product[style_cards][<?= $styleIndex ?>][current_image]" value="<?= admin_html($styleImage) ?>">
                              <div class="admin-grid one-up">
                                <?php admin_input('product[style_cards][' . $styleIndex . '][label]', 'Style Name', $styleLabel, 'text', '', 'Used for display, and the URL/filter value is created from this name automatically.'); ?>
                              </div>
                              <div class="admin-grid two-up">
                                <?php admin_input('style_card_image_url_' . $styleIndex, 'Image URL', $styleImage, 'text', '', 'Paste an image URL or upload one below.'); ?>
                                <label class="admin-field">
                                  <span>Upload Image</span>
                                  <input type="file" name="style_card_image_file_<?= $styleIndex ?>" accept="image/*">
                                </label>
                              </div>
                            </div>
                          <?php endforeach; ?>
                        </div>
                        <button class="admin-add" type="button" data-add-item data-template="tpl-product-style-card">Add Style</button>
                      </div>
                    </div>
                    <?php
                      $engagementSectionCards = array_values($attributeProfile['style_cards_sections']['engagement'] ?? []);
                      $weddingSectionCards = array_values($attributeProfile['style_cards_sections']['wedding'] ?? []);
                      if ($weddingSectionCards === []) {
                          $weddingSectionCards = array_values(default_attribute_profiles()['Ring']['style_cards_sections']['wedding'] ?? []);
                      }
                    ?>
                    <div class="admin-field admin-field-full" id="attribute-engagement-styles">
                      <span>Engagement Rings — Shop by Style</span>
                      <p class="admin-table-note">Styles shown in the Engagement Rings section. Leave empty to use the shared Ring Style Showcase list above.</p>
                      <div class="admin-repeater" data-repeater data-index-token="__ENG_STYLE_INDEX__" data-next-index="<?= count($engagementSectionCards) ?>">
                        <div class="admin-repeater-list">
                          <?php foreach ($engagementSectionCards as $engStyleIndex => $engStyleCard): ?>
                            <?php
                              $engStyleLabel = clean_string((string) ($engStyleCard['label'] ?? ''), 120);
                              $engStyleImage = clean_image((string) ($engStyleCard['image'] ?? ''));
                            ?>
                            <div class="admin-repeater-item compact-item">
                              <div class="admin-item-head"><h4><?= admin_html($engStyleLabel !== '' ? $engStyleLabel : 'Style ' . ($engStyleIndex + 1)) ?></h4><button class="admin-remove" type="button" data-remove-item>Delete</button></div>
                              <input type="hidden" name="product[style_cards_sections][engagement][<?= $engStyleIndex ?>][current_image]" value="<?= admin_html($engStyleImage) ?>">
                              <div class="admin-grid one-up">
                                <?php admin_input('product[style_cards_sections][engagement][' . $engStyleIndex . '][label]', 'Style Name', $engStyleLabel, 'text', '', 'e.g. Solitaire, Halo, Toi et Moi.'); ?>
                              </div>
                              <div class="admin-grid two-up">
                                <?php admin_input('style_card_engagement_image_url_' . $engStyleIndex, 'Image URL', $engStyleImage, 'text', '', 'Paste an image URL or upload one below.'); ?>
                                <label class="admin-field">
                                  <span>Upload Image</span>
                                  <input type="file" name="style_card_engagement_image_file_<?= $engStyleIndex ?>" accept="image/*">
                                </label>
                              </div>
                            </div>
                          <?php endforeach; ?>
                        </div>
                        <button class="admin-add" type="button" data-add-item data-template="tpl-product-style-card-engagement">Add Engagement Style</button>
                      </div>
                    </div>
                    <div class="admin-field admin-field-full" id="attribute-wedding-styles">
                      <span>Wedding Rings — Shop by Style</span>
                      <p class="admin-table-note">Styles shown in the Wedding Rings section (both Men's and Women's). These are separate from engagement styles.</p>
                      <div class="admin-repeater" data-repeater data-index-token="__WED_STYLE_INDEX__" data-next-index="<?= count($weddingSectionCards) ?>">
                        <div class="admin-repeater-list">
                          <?php foreach ($weddingSectionCards as $wedStyleIndex => $wedStyleCard): ?>
                            <?php
                              $wedStyleLabel = clean_string((string) ($wedStyleCard['label'] ?? ''), 120);
                              $wedStyleImage = clean_image((string) ($wedStyleCard['image'] ?? ''));
                            ?>
                            <div class="admin-repeater-item compact-item">
                              <div class="admin-item-head"><h4><?= admin_html($wedStyleLabel !== '' ? $wedStyleLabel : 'Style ' . ($wedStyleIndex + 1)) ?></h4><button class="admin-remove" type="button" data-remove-item>Delete</button></div>
                              <input type="hidden" name="product[style_cards_sections][wedding][<?= $wedStyleIndex ?>][current_image]" value="<?= admin_html($wedStyleImage) ?>">
                              <div class="admin-grid one-up">
                                <?php admin_input('product[style_cards_sections][wedding][' . $wedStyleIndex . '][label]', 'Style Name', $wedStyleLabel, 'text', '', 'e.g. Classic Band, Eternity, Pavé.'); ?>
                              </div>
                              <div class="admin-grid two-up">
                                <?php admin_input('style_card_wedding_image_url_' . $wedStyleIndex, 'Image URL', $wedStyleImage, 'text', '', 'Paste an image URL or upload one below.'); ?>
                                <label class="admin-field">
                                  <span>Upload Image</span>
                                  <input type="file" name="style_card_wedding_image_file_<?= $wedStyleIndex ?>" accept="image/*">
                                </label>
                              </div>
                            </div>
                          <?php endforeach; ?>
                        </div>
                        <button class="admin-add" type="button" data-add-item data-template="tpl-product-style-card-wedding">Add Wedding Style</button>
                      </div>
                    </div>
                  <?php else: ?>
                    <div class="admin-field admin-field-full" id="attribute-collection-showcase">
              ...
              <div class="admin-product-main">
                <?php if ($attributeTypeIsRing): ?>
                  <section class="admin-product-card">
                    ...
                    <p class="admin-table-note">Style card images and style display names are managed above in the <a href="#attribute-ring-styles">Ring Styles</a> section.</p>
                    <div class="admin-grid two-up">
                      <div class="admin-field admin-field-full">
      ...
      <?php elseif ($view === 'customers'): ?>
        <section class="admin-page-hero"><div><p class="admin-kicker">Registered Users</p><h2>Accounts</h2><p>Search and moderate customers from one queue instead of scanning the full account table.</p></div><div class="admin-mini-stats"><article><span>Total Users</span><strong><?= count($content['customers']['items']) ?></strong></article><article><span>Banned</span><strong><?= count(array_filter($content['customers']['items'], static fn (array $item): bool => strtolower((string) ($item['status'] ?? '')) === 'banned')) ?></strong></article><article><span>Filtered</span><strong><?= count($filteredCustomers) ?></strong></article></div></section>
        <section class="admin-panel">
      ...
      <?php elseif ($view === 'site'): ?>
        ...
        <section class="admin-panel" id="site-hero"><div class="admin-panel-head"><div><p class="admin-kicker">Homepage</p><h3>Hero Section</h3></div></div>
          ...
          <div class="admin-grid two-up">
            <?php admin_input('hero_image_url', 'Background Media URL', '', 'text', '', 'Paste a URL or upload a file (max 10MB video/image).'); ?>
            <label class="admin-field">
              <span>Upload Media (replaces current)</span>
