        ...
        <div class="mnav-item has-mega">
          ...
          <?php if ($isRingMega): ?>
            ...
            <?php $megaStyleCol = ob_get_clean(); ob_start(); ?>
                <?php
                  // Tagline shown under each metal name in the redesigned
                  // "Shop by Metal" list. Any metal the admin adds that isn't
                  // in this map still renders perfectly via the neutral default.
                  $navMetalTagline = static function (string $name): string {
                      $n = strtolower($name);
                      if (str_contains($n, 'rose')) return 'Romantic & Elegant';
                      if (str_contains($n, 'platinum')) return 'Rare & Radiant';
                      if (str_contains($n, 'white')) return 'Modern & Sophisticated';
                      if (str_contains($n, 'yellow') || (str_contains($n, 'gold') && !str_contains($n, 'white') && !str_contains($n, 'rose'))) return 'Timeless & Classic';
                      return 'Premium Metal';
                  };
                  // Pick a legible gem-icon colour for ANY disc colour the admin
                  // sets: white on dark discs, deep ink on light ones (so a brand
                  // new metal with an unusual hex still reads cleanly).
                  $navMetalGemColor = static function (string $hex): string {
                      $hex = ltrim($hex, '#');
                ...
                <div class="mega-col mega-col-collections mega-col-metal">
                  ...
                  <?php foreach ($navRingMetals as $navMetal): ?>
                    <a href="<?php e(resolve_link('/shop/?' . http_build_query($ringSectionQuery + ['color' => $navMetal['label']]))); ?>" class="mega-metal-link">
                      <span class="mega-metal-disc" style="background-color: <?= h($navMetal['hex']); ?>;">
                        <i class="fas fa-gem mega-metal-gem" style="color: <?= h($navMetalGemColor($navMetal['hex'])); ?>;"></i>
                      </span>
                      <span class="mega-metal-text">
                        <span class="mega-metal-name"><?php e($navMetal['label']); ?></span>
                        <span class="mega-metal-tag"><?php e($navMetalTagline($navMetal['label'])); ?></span>
                      </span>
