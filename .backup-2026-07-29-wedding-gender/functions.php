...
function available_ring_style_cards(string $ringCategory = ''): array
{
    ...
    // Section-specific cards (engagement / wedding) win when present, then the
    // shared profile list, then the hard-coded engagement defaults.
    $profileCards = [];
    if ($ringCategory !== '' && isset($profile['style_cards_sections'][$ringCategory]) && is_array($profile['style_cards_sections'][$ringCategory])) {
    ...
    if ($profileCards === []) {
        ...
        if ($ringCategory !== '' && isset($ringsProfile['style_cards_sections'][$ringCategory]) && is_array($ringsProfile['style_cards_sections'][$ringCategory])) {
    ...
    // The shared engagement defaults only backfill the engagement/all-rings view;
    // wedding keeps exactly the cards configured for its section.
...
function product_ring_taxonomy(array $product): array
{
    ...
    if (!in_array($gender, ['mens', 'womens', 'unisex'], true)) {
        $gender = '';
    }
...
function product_category_label(array $product): string
{
    ...
    if ($taxonomy['category'] !== '') {
        ...
        if ($taxonomy['category'] === 'wedding' && $taxonomy['gender'] !== '' && $taxonomy['gender'] !== 'unisex') {
...
/**
 * The unified category choices offered on the product form. Each choice maps
 * to the exact product_type + ring_category + ring_gender combination, so one
 * dropdown decides where the product lives on the storefront.
 */
function product_category_taxonomy_options(): array
{
    return [
        ...
        'wedding-unisex' => ['label' => 'Wedding Rings — Unisex', 'product_type' => 'Rings', 'ring_category' => 'wedding', 'ring_gender' => 'unisex'],
        'Earring' => ['label' => 'Earrings', 'product_type' => 'Earring', 'ring_category' => '', 'ring_gender' => ''],
        'Pendant' => ['label' => 'Pendants', 'product_type' => 'Pendant', 'ring_category' => '', 'ring_gender' => ''],
        'Bracelet' => ['label' => 'Bangles & Bracelets', 'product_type' => 'Bracelet', 'ring_category' => '', 'ring_gender' => ''],
        'Necklace' => ['label' => 'Necklaces', 'product_type' => 'Necklace', 'ring_category' => '', 'ring_gender' => ''],
        'Brooch' => ['label' => 'Brooches', 'product_type' => 'Brooch', 'ring_category' => '', 'ring_gender' => ''],
        'Jewellery Set' => ['label' => 'Jewellery Sets', 'product_type' => 'Jewellery Set', 'ring_category' => '', 'ring_gender' => ''],
        'Mangalsutra' => ['label' => 'Mangalsutra', 'product_type' => 'Mangalsutra', 'ring_category' => '', 'ring_gender' => ''],
    ];
}

/**
 * The taxonomy option key a product currently maps to (for preselecting the
 * Category dropdown). Unknown/custom types fall back to their product_type.
 */
function product_category_taxonomy_key(array $product): string
{
    ...
    if ($taxonomy['category'] === 'wedding') {
        return 'wedding-' . ($taxonomy['gender'] !== '' ? $taxonomy['gender'] : 'unisex');
    }

    return clean_string((string) ($product['product_type'] ?? ''), 80);
}
...
function render_shop_listing_card(array $product): void
{
    ...
    $swatchToneFor = static function (string $label): string {
    ...
    foreach ((array) ($product['metal_variations'] ?? []) as $variation) {
        if (($variation['active'] ?? false) && (string) ($variation['metal'] ?? '') !== '') {
            $swatchLabels[] = (string) $variation['metal'];
        }
    }
    if ($swatchLabels === [] && (string) ($product['color'] ?? '') !== '') {
        $swatchLabels[] = (string) $product['color'];
    }
    $swatchTones = [];
    foreach ($swatchLabels as $swatchLabel) {
        $tone = $swatchToneFor($swatchLabel);
        if (!in_array($tone, $swatchTones, true) && count($swatchTones) < 4) {
            $swatchTones[] = $tone;
        }
    }
    ...
    <div class="sl-card">
      <div class="sl-card-media">
        ...
        <div class="sl-card-swatches" aria-hidden="true">
          <?php foreach ($swatchTones as $tone): ?><span class="sl-swatch sl-swatch--<?= h($tone) ?>"></span><?php endforeach; ?>
        </div>
        <?php endif; ?>
      </div>
