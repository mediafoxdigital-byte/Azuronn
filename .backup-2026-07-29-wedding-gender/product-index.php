    ...
    <nav class="pp-breadcrumb" aria-label="Breadcrumb">
      ...
      <?php if ($productTaxonomy['category'] !== ''): ?>
        ...
        <?php if ($productTaxonomy['gender'] !== ''): ?>
          <a href="<?= h($crumbGenderUrl) ?>"><?= h($productTaxonomy['gender'] === 'mens' ? "Men's" : ($productTaxonomy['gender'] === 'womens' ? "Women's" : 'Unisex')) ?></a>
          <span class="pp-crumb-sep">/</span>
        <?php endif; ?>
      <?php else: ?>
            ...
            <div class="pp-opt">
              ...
              <div class="pp-grid-opt" id="band-claw-options-grid">
                <?php foreach ($selectedMetalBandOptions as $bandMetal): ?>
                  <?php $bandTone = $optionToneFor((string) ($bandMetal['label'] ?? '')); $bandSurcharge = (float) ($bandMetal['surcharge'] ?? 0); ?>
                  <label class="option-card option-card-band tone-<?= h($bandTone) ?>">
                    ...
                    <span class="option-card-band-thumb" aria-hidden="true"><?= $renderProductMedia($productPrimaryMedia, (string) ($product['name'] ?? 'Product'), 'option-card-band-image', [], false) ?></span>
          ...
          <div class="pp-summary">
            ...
            <div class="pp-cta">
              <?php if ($isEngagementRing): ?>
                ...
                <a href="<?= h(resolve_link('/contact/')) ?>" class="pp-appointment"><i class="far fa-calendar"></i> Request an appointment</a>
              <?php else: ?>
