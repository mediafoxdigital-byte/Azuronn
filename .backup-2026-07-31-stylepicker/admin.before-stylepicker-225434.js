...
function syncProductEditorScopes(scope = document) {
  scope.querySelectorAll('select[name="product[product_type]"]').forEach((select) => {
    const form = select.closest('form');
    if (!form) return;
...
function syncProductStyleSection(scope = document) {
  scope.querySelectorAll('[data-style-picker-wrap]').forEach((wrap) => {
    ...
    if (categorySelect) {
      try {
        map = JSON.parse(categorySelect.dataset.categoryStyleMap || '{}');
      } catch (_error) {
        map = {};
      }
    }

    let section = categorySelect ? (map[categorySelect.value || ''] || '') : '';
    if (section !== 'engagement' && section !== 'wedding') {
      section = 'engagement'; // default for ring / fresh forms
    }

    const wrapHidden = wrap.hidden || wrap.style.display === 'none';
    wrap.querySelectorAll('[data-style-section]').forEach((grid) => {
      const isActive = !wrapHidden && grid.dataset.styleSection === section;
      grid.style.display = isActive ? '' : 'none';
      // Disable every input in the inactive grid — checkboxes AND the empty
      // sentinel — so a hidden section contributes nothing to the save. (The
      // scope-sync loop above re-enables all controls in a visible wrap, so we
      // must re-assert the per-grid disable here, after it.)
