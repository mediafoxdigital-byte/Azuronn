...
new_css = """<style>
  ...
  .ring-journey-overview {
    background: transparent;
    border-bottom: none;
    margin-top: -33px; /* Pull banner up */
    position: relative;
    z-index: 5;
  }
  .premium-step-banner {
  ...
  /* Create absolute link for whole card */
  .shop-page .prod-name a::before {
    content: "";
    position: absolute;
    inset: 0;
    z-index: 5;
  ...
  .shop-page .prod-card:hover .prod-name a::after {
    background: #fcf4e6;
    color: #1a1a1a;
  }
...
# Replace hero HTML
old_hero = """<section class="collection-hero reveal-in <?= $showRingJourney ? 'ring-journey-hero' : '' ?>" style="padding-bottom: 0;">
  <div class="container">
    <h1><?= h($heroTitle) ?></h1>
...
new_hero = """<section class="collection-hero reveal-in <?= $showRingJourney ? 'ring-journey-hero' : '' ?>" style="padding-bottom: 0;">
  <div class="container">
    <h1><?= h($heroTitle) ?><?php if ($showRingJourney): ?><span>✦</span><?php endif; ?></h1>
    <p><?= h($heroDescription) ?></p>
    <?php if ($showRingJourney): ?>
    <div class="premium-hero-badge">
      <svg viewBox="0 0 100 100">
        <path id="badge-curve" d="M 50 15 A 35 35 0 1 1 49.9 15" fill="transparent"/>
        <text font-size="8" font-family="var(--sans, sans-serif)" font-weight="600" fill="#c9a96e" letter-spacing="0.1em">
          <textPath href="#badge-curve" startOffset="50%" text-anchor="middle">TIMELESS ELEGANCE • PREMIUM QUALITY • </textPath>
        </text>
      </svg>
      <i class="far fa-gem center-icon"></i>
    </div>
    <?php endif; ?>
  </div>
</section>"""
