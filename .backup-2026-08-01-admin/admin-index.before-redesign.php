            ...
            <div class="admin-product-editor" data-catalog-profile-editor>
              <div class="admin-product-main">
                <section class="admin-product-card">
                  ...
                  // Hide dead/legacy categories (Brooch, Jewellery Set) from the
                  // dropdown for NEW uploads — they have no real catalogue presence
                  // and confuse merchants. But keep them resolvable when EDITING a
                  // product that already belongs to one (so editing never goes blank).
                  $legacyOnlyCategories = ['Brooch', 'Jewellery Set'];
                  $editingTaxonomyKey = $editingProduct !== null ? product_category_taxonomy_key($editingProduct) : '';
                  foreach (product_category_taxonomy_options() as $taxonomyKey => $taxonomyOption) {
                      if (in_array($taxonomyKey, $legacyOnlyCategories, true) && $taxonomyKey !== $editingTaxonomyKey) {
                          continue;
                      }
                ...
                <section class="admin-product-card" data-product-scope="ring, rings, earring, earrings, bracelet, bangles & bracelets, necklace, necklaces, pendant, pendants">
                  ...
                  <?php
                    ...
                    $flatStyleTypes = ['Earring', 'Pendant', 'Bracelet', 'Necklace', 'Mangalsutra'];
                    $initialFlatType = '';
                    if ($editingProduct !== null) {
                        $editType = admin_canonical_attribute_type((string) ($editingProduct['product_type'] ?? ''));
                        if (!in_array($editType, ['Ring', 'Rings'], true) && in_array($editType, $flatStyleTypes, true)) {
