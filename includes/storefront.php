...
function checkout_place_order(array $input): array
{
    ...
    return site_content_with_lock(static function () use (
        ...
        // Created AFTER stock validation but BEFORE save_site_content so that a
        // Stripe API failure doesn't leave a half-saved order or clear the cart.
        if ($isStripeOrder) {
            ...
            // Tag the order with the session ID so the webhook / success page
            // can find it. The order is still pending/awaiting at this point.
            $orderIndex = array_key_last($content['orders']['items']);
            $content['orders']['items'][$orderIndex]['stripe_checkout_session_id'] = (string) ($stripeResult['session_id'] ?? '');
